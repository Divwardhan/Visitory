...
const appointmentRouter = (db) => {
    ...
    router.post("/accept-appointment", guardmiddleware, async (req, res) => {
        try {
            ...
            var id= null
            if(appointmentRows[0].requested_by =="faculty"){
                id = "fid"
            }
            else if(appointmentRows[0].requested_by =="adminstaff"){
                id='wid'
            }
            else{
                id = "sid"
            }
            const [phoneNumber] = await req.db.query(`SELECT phone FROM ${appointmentRows[0].requested_by} WHERE ${id} = ?`, [appointmentRows[0].appointed_by_id]);
            console.log(phoneNumber[0].phone);
            const message = `Your appointment with ${appointmentRows[0].visitor_name} has been accepted.`;
            const notification = await sendNotification("+91"+phoneNumber[0].phone, message);
            console.log("h1");
